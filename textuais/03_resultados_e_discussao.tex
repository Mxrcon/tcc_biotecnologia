...
\begin{figure}[H]
	...
	\centering
	\legend{Box plot representando a variação média de qualidade das leituras (eixo Y) em relação a posição
	da base na leitura $($eixo X$)$, a linha azul representa a média, \textit{box-plots} representando a distribuição
	em cada base. As cores verde, amarelo e vermelho representam respectivamente: qualidade excelente $($\textgreater$)$,
